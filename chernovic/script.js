...
//получим все формы по тегу form
const message = { // объект с сообщениями
    loading: 'Загрузка...',
...
forms.forEach(item => { // Берем все формы и под каждую из них подвязываем postData
    postData(item);
});

function postData(form) {
    // функция постит данные и принимает какую то форму для навешивания обработчика события
    form.addEventListener('submit', (e) => {
        // при событии submit, который сработает при нажатии на кнопку на сайте 
        e.preventDefault();
        // отменяем стандартное поведение браузера, перезагрузка страници при отправки формы


        const statusMessage = document.createElement('div');
        // создаем блок для сообщения, добавляя его к форме 
        statusMessage.classList.add('status');// добавляем классы
        statusMessage.textContent = message.loading;
        // берем элемент и во внутрб помещаем то сообщение,который мы  хотим показать
        form.append(statusMessage); // отправка на страницу


        const request = new XMLHttpRequest(); 
        // создаем объект request методом XMLHttpRequest
        request.open('POST', 'server.php');
        // вызываем метод open, чтобы настроить этот запрос

        //request.setRequestHeader('Content-type', 'multipart/form-data');
        //заголовок на сервере устанавливаеться автоматично и самому не надо, будет ошибка 
        // настройка заголовков, говорим серверу, что именно приходит 
        const formData = new FormData(form);
        // во внутрь помешаем ту форму из которой нужно собрать данные
        // при отправки данных на сервер обязательно указываем атрибут name = "name"(называем поле формы) в HTML,чтобы найти этот input и взять из него value для того, чтоюы сформировать из него правельный объект 
        //<form action="#" class="order__form">
       // <input required placeholder="Ваше имя" name="name" type="text" class="order__input">

       request.send(formData); // отправляем дынные на сервер

       request.addEventListener('load', () => {
           if (request.status === 200) {
               console.log(request.response);
               statusMessage.textContent = message.success; // сообщение
           } else {
               statusMessage.textContent = message.faolure;
           }
           // после хорошо сообщить пользователю, что происходит обрабатывая ненативные и позитивные сценории
       });
       //отслеживает конечную загрузку нашего запроса
    });
}
